...
sudo docker run -d --network=host --add-host=host.docker.internal:host-gateway -v open-webui:/app/backend/data --name open-webui --restart no ghcr.io/open-webui/open-webui:main
